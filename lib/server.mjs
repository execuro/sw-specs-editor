...
export class Session {
  ...
  tick() {
    ...
    if (this.run && !this.run.warned && this.lastPoll > 0 && t - this.lastPoll > this.opts.agentTimeout * 1000) {
      ...
      this.chatAppend({ type: 'system', text: `no sign of the agent for ${Math.round(this.opts.agentTimeout / 60)} min; still waiting. Abort the run from the page if it is stuck.` });
    }
  }
  polled() {
