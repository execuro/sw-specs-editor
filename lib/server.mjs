...
export class Session {
  ...
  info() {
    return {
      ...
      locks: this.locks, run: this.run ? { id: this.run.id, docs: this.run.docs, kind: this.run.kind, silent: Boolean(this.run.silent) } : null, queue: this.queue.map(b => b.id),
  ...
  reply(body) {
    ...
    if (!this.run.pending.size) this.finishRun();
    return { ok: true, results };
  }

  /**
   * Common tail of reply and abort: clear the run slot and wake the poll loop so
   * the next queued batch can start. Per-document unlocking is `finishDoc`'s job,
   * so every pending document must be finished before this runs.
   */
  finishRun(extra = {}) {
    const finished = this.run.id;
    this.run = null;
    this.broadcast('run', { state: 'finished', batch: finished, locks: this.locks, queue: this.queue.map(b => b.id), active: null, ...extra });
    this.saveState();
    this.saveQueueManifest();
   ...
   * worker died, the terminal closed. Without this the documents stay locked and
  ...
  abortRun(reason) {
    ...
    for (const d of [...this.run.pending]) this.finishDoc(d, { markdown: `Run aborted: ${reason}`, text: '' }, id);
  ...
  clearCache() {
    ...
    this.broadcast('run', { state: 'finished', batch: null, locks: this.locks, queue: [], active: null });
    return { ok: true };
  }
  ...
  tick() {
    ...
    if (this.run && !this.run.warned && this.lastPoll > 0 && t - this.lastPoll > RUN_SILENCE * 1000) {
      ...
      this.broadcast('run', { state: 'silent', batch: this.run.id, locks: this.locks, queue: this.queue.map(b => b.id), active: this.run.id, silent: true });
    }
  }
  polled() {
    ...
    if (this.run?.silent) {
      ...
      this.broadcast('run', { state: 'started', batch: this.run.id, locks: this.locks, queue: this.queue.map(b => b.id), active: this.run.id });
    }
