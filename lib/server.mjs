...
// Until a tab has ever checked in, the grace period is not "the page went away"
// but "the user has not opened the URL yet" - a different, much longer wait.
// `--grace` still governs everything after the first heartbeat.
const OPEN_GRACE = 600;
...
export class Session {
  ...
  // --- lifecycle
  beat() { this.lastBeat = Date.now(); this.everBeat = true; }
  tick() {
    const t = Date.now();
    const grace = this.everBeat ? this.opts.grace : Math.max(this.opts.grace, OPEN_GRACE);
    if (t - this.lastBeat > grace * 1000) { this.shutdown('heartbeat timeout'); return; }
