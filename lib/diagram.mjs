...
import { sessionSlugFor } from './parse.mjs';
...
/**
 * Default SVG location: the session folder of the document the diagram belongs
 * to. The server serves each fallback out of its own session folder, so the
 * document's mode has to be part of the name - `sessionSlugFor` owns that rule.
 */
export function defaultSvgPath(outFile) {
  const dir = path.dirname(outFile);
  const parts = path.basename(outFile).replace(/\.excalidraw$/i, '').split('.');
  const name = parts.length > 1 ? parts.pop() : 'diagram';
  return path.join(dir, '.editor', sessionSlugFor(`${parts.join('.')}.md`), `${name}.svg`);
}
