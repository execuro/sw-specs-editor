 ...
 */
export function settle(o, { cwd = process.cwd(), docIsPath = true } = {}) {
  if (Array.isArray(o.docs)) o.docs = o.docs.map(d => canonical(d, cwd));
  // `emit --doc` names a side of the pair (`prd` / `spec`), not a file. Turning
  // that into a path would invent a document that does not exist.
  if (docIsPath && typeof o.doc === 'string' && o.doc) o.doc = canonical(o.doc, cwd);
  const anchor = (Array.isArray(o.docs) && o.docs[0]) || (docIsPath ? o.doc : '') || '';
