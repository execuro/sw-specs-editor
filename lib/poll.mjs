...
export async function main(argv, { resolvePair } = {}) {
  ...
  if (opts.reply) {
    // Same endpoint `emit done` uses. With no doc-scoped target the server
    // replies for every pending document; with no run active it records an
    // orphan chat entry and this still falls through to the wait loop below.
    let res;
    try {
  ...
  try {
    while (Date.now() < deadline) {
      ...
      if (body.event === 'batch') {
        ...
        out.nextStep(`read the batch file, do the work, then\n\`sw-specs-editor emit done --batch ${body.batch?.id || '<id>'} --doc <prd|spec> -\``);
