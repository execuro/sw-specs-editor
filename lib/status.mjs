...
function questionBlockMarkdown(row) {
  ...
  for (const n of questionNotes) lines.push(`  - [${n.agent}] ${n.text}`);
  for (const o of row.options) {
    lines.push(`- [ ] ${o.id}: ${o.text}${o.recommended ? ' (recommended)' : ''}`);
    for (const n of byOption.get(o.id)) lines.push(`  - [${n.agent}] ${n.text}`);
  }
  return lines.join('\n');
}
...
export function setAnswer(text, id, answer, opts = {}) {
  ...
  if (answer && answer.option) {
    ...
    else { lines.splice(lastTopLevelIdx + 1, 0, wantLine); changed = true; }
  } else if (ownIdx >= 0) {
