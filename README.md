...
- Every question the agent raises arrives advised: one `(recommended)` option with at least one agent note behind it. The only unadvised questions are one the agent looked at and declined (`  - [pm] no recommendation — business preference`) and one the user raised themselves (`  - [user] raised on the page — advice not requested`); anything else is listed in `questions.unadvised` in the next batch and shown as **advice pending** on the card. The `(recommended)` mark and the notes are snapshotted before a run and restored if it drops them.
