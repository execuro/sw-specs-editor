...
allowed-tools: Read Glob Grep Skill Bash(npx -y @execuro-sw-ecosystem/sw-specs-editor@0.1.3 *) Bash(open *) Bash(xdg-open *) Bash(grep *) Bash(printf *) Bash(tail *)
license: MIT
metadata:
  author: Execuro UG (haftungsbeschränkt)
  package: "@execuro-sw-ecosystem/sw-specs-editor@0.1.3"
---
...
It is the single source for start, open, poll, emit and close, for the poll rules, and for the batch's fields. Every command also ends with a `next_step:` line; follow it. The CLI prints its own name bare (`sw-specs-editor poll`) — run each one as `npx -y @execuro-sw-ecosystem/sw-specs-editor@0.1.3 poll`.
...
- A run that spawns a helper agent can stay silent for a while. The server never aborts a run for silence, but emit progress before and after every spawn so the page's presence indicator stays alive. A run that raises questions posts its anchored report with `emit chat` first and keeps working while it gets those questions advised, so an interim chat entry before the reply is the normal shape, not a failure — the reply that follows carries the recommendations. If a run's agent is gone for good (the session was interrupted), the user ends it with **Abort run** on the page; the document unlocks and whatever they queued meanwhile is applied.
